...
st.markdown("""
    <style>
        ...
        /* Main app container background gradient */
        .stApp {
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
            color: #f5f5f7; /* light off-white */
            padding: 15px 25px;
        }
        ...
        div[style*='text-align: center; font-size: 22px; font-weight: bold; padding-top: 10px;'] {
            color: #e0e0e8;
        ...
        /* Matplotlib plot text and background */
        .stPlotlyChart, .stImage {
            border-radius: 12px;
            box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
        }

        /* For warning, success messages */
        .stAlert {
            border-radius: 8px;
            padding: 12px 20px;
            font-weight: 600;
        ...
        /* Customize the dropdown and multiselect */
        .css-1v3fvcr, .css-1hwfws3 {
...
st.markdown("""
    <div style='text-align: left; font-size: 22px; padding-top: 10px; color: #FFFFFF;'>
        Select a country and see the forecasted EV adoption trend for the next 3 years.
    </div>
""", unsafe_allow_html=True)
...
county = st.selectbox("Select a Country", county_list)
...
st.subheader(f"📊 Cumulative EV Forecast for {county} Country")
...
multi_counties = st.multiselect("Select up to 3 countries to compare", county_list, max_selections=3)
...
if multi_counties:
    ...
    for cty, group in comp_df.groupby('Country'):
    ...
    growth_summaries = []
    for cty in multi_counties:
        cty_df = comp_df[comp_df['Country'] == cty].reset_index(drop=True)
